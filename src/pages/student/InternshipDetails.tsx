...
import { applicationService, internshipService, companyService } from "../../api";
...
export const InternshipDetails = () => {
  ...
  const hasApplied = (applications ?? []).some((application) => application.internshipId === internshipId);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white border-2 border-black p-6 shadow-[6px_6px_0px_black] rounded flex flex-col gap-6 h-fit">
          ...
          {loadingCompany ? (
            ...
            <>
              <div>
                ...
                <div className="border border-black px-4 py-2 rounded bg-gray-100">{company?.industry || "N/A"}</div>
              </div>

              <div>
                <p className="text-sm text-gray-500 mb-1">COMPANY SIZE</p>
                <div className="border border-black px-4 py-2 rounded bg-gray-100">{company?.companySize || "N/A"}</div>
              </div>

              <div>
                <p className="text-sm text-gray-500 mb-1">FOUNDED</p>
                <div className="border border-black px-4 py-2 rounded bg-gray-100">{company?.foundedYear || "N/A"}</div>
              </div>
          ...
          <button
            ...
            disabled={hasApplied}
            className="mt-4 bg-[#5D0CA0] text-white border-2 border-black px-6 py-3 shadow-[4px_4px_0px_black] rounded hover:translate-y-[2px] hover:shadow-none transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <ApplyModal
            open={!!openApply && !hasApplied}
            title={internship.title}
            onClose={() => setOpenApply(false)}
            onSubmit={async (coverLetter: string) => {
              await applicationService.submit(internship.id, coverLetter);
